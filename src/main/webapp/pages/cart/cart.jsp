        ...
        $(function () {
            ...
            $("input.updateCountClass").change(function () {
                ...
                if (confirm("您确定要将【" + itemName + "】数量修改为 " + itemCount + " 吗？")) {
                    location.href = "${basePath}cartServlet?action=updateCount&bookId=" + bookId + "&newCount=" + itemCount;
                } else {
...
<div id="main">
    ...
    <c:if test="${empty sessionScope.cart.items}">
        <h1 href="index.jsp" style="text-align: center; padding: 222px">您的购物车中暂无数据，赶快返回商城浏览商品吧！</h1>
    </c:if>
        ...
        <table>
            ...
            <c:forEach items="${sessionScope.cart.items}" var="entry">
                <tr>
                    ...
                    <td><label><input bookId="${entry.value.id}" class="updateCountClass" style="width: 66px; text-align: center" type="text" value="${entry.value.count}"></label></td>
                    <td>${entry.value.price}</td>
                    <td>${entry.value.totalPrice}</td>
                    <td><a class="deleteItemClass" href="cartServlet?action=deleteItem&bookId=${entry.value.id}">删除</a></td>
                </tr>
            </c:forEach>
        </table>
        <div class="cart_info">
